...
- By default, logs hide all header values except content type, length, encoding and accept. They hide query strings and recursively redact credential fields, including JSON-encoded tool arguments. Known header, query and body credentials are also scrubbed from repeated text. Common API-key, bearer/basic, JWT, private-key and credential-assignment patterns are scrubbed.
- Use `--no-redact` to disable redaction in pretty, JSON and JSONL output. Headers, paths, query strings and inspected JSON bodies will include credentials unchanged. This does not change forwarding, body size limits or response logging. Terminal escape sequences are still sanitized. Treat these logs as credentials.
...
- JSON request inspection supports plain, gzip, deflate, Brotli and zstd bodies. Both compressed and decoded inspection size are bounded by `--max-body`, default 16 MiB. Oversized, invalid or non-JSON bodies are omitted from logs but still forwarded unchanged. Redaction also omits bodies that exceed its nesting limit. A visible `BODY NOT SHOWN` section explains omissions. Increase the limit, up to 64 MiB, if full context requests are omitted.
