# Set up skim
export SKIM_DEFAULT_OPTIONS="--color light,batched_bg:-1 --tiebreak begin --ansi --no-mouse --tabstop 4 --inline-info"
...
skim-history-widget() {
  ...
  selected=( $(fc -rl 1 | perl -ne 'print if !$seen{(/^\s*[0-9]+\**\s+(.*)/, $1)}++' |
    SKIM_DEFAULT_OPTIONS="--color light,matched_bg:-1 --height ${SKIM_TMUX_HEIGHT:-40%} --ansi --no-mouse --tabstop 4 --inline-info -n2..,.. --tiebreak=index --bind=ctrl-r:toggle-sort $SKIM_CTRL_R_OPTS --query=${(qqq)LBUFFER} --no-multi" $(__skimcmd)) )
  local ret=$?
  if [ -n "$selected" ]; then
    num=$selected[1]
    if [ -n "$num" ]; then
